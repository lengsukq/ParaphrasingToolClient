...
这是一款基于Vue 3和TypeScript开发的文本查重与AI降重工具，主要用于解析大雅、PaperYY 查重报告和 PaperYY AIGC 检测报告，并支持用户自定义大模型及Prompt进行文本降重。同时提供 AI 智能解析模式，可自动分析任意格式查重报告并生成解析规则。该工具提供了友好的用户界面和高效的文本处理功能，旨在帮助用户快速有效地降低文本重复率。
...
### 1. 多种查重报告支持
*   **论文降重功能**：支持上传大雅、PaperYY 查重报告（HTML格式），自动解析报告内容，清晰展示原文、相似源及修改建议。
*   **AIGC检测功能**：支持上传 PaperYY 的 AIGC 检测报告（HTML格式），分析文本由AI生成的可能性，定位高风险段落并支持一键降重。
*   **AI智能解析**：通过 AI 自动分析查重报告结构并生成解析规则，兼容任意来源、任意格式的查重报告。支持规则保存、重命名、导入/导出（ZIP）。

### 2. 智能AI降重
*   **一键降重**：针对检测出的高重复率文本，提供便捷的一键AI降重功能。
*   **批量降重**：支持一键对所有未降重条目批量执行 AI 降重。
*   **实时结果展示**：降重后的文本即时内联显示，并提供复制和全屏查看功能。
...
*   **可折叠文本**：表格内长文本默认显示 3 行，超出可展开/收起。
*   **空状态引导**：上传前提供操作指引和格式说明。
*   **响应式设计**：界面自适应不同设备的屏幕尺寸。
*   **现代UI**：采用 shadcn-vue + Tailwind CSS 设计系统，界面美观、简洁、易用。
...
| 路由 | Vue Router 4 |
| AI 调用 | OpenAI SDK（兼容任意 OpenAI API 规范服务） |
| 工具库 | VueUse、JSZip、file-saver |
| 通知 | vue-sonner |
| 部署 | Cloudflare Pages（前端静态托管） |
| AI 服务中转 | Cloudflare Worker（取代已弃用的 Python 后端） |

> 原 Python 后端（[开源地址](https://github.com/lengsukq/ParaphrasingToolServer)）已停止维护。

### AI 模型调用逻辑
若用户未在前端「系统配置」中进行有效配置，AI 降重将尝试使用 Cloudflare Worker 的后备默认配置进行（详见上一节“Cloudflare Worker 后备默认配置”）。
...
### 2. 上传查重报告
1.  在顶部导航栏选择「论文降重」、「AIGC检测」或「AI智能解析」功能标签。
2.  将查重报告文件（目前仅支持HTML格式）拖拽到上传区域，或点击上传区域选择文件。
3.  系统会在您的浏览器本地快速解析报告内容，并在表格中显示结果。

> **提示**：如果您的查重报告格式不被「论文降重」或「AIGC检测」支持，请使用「AI智能解析」模式，AI 将自动分析并生成解析规则。

### 3. 使用AI降重
1.  在解析结果表格中，找到需要降重的文本行。
2.  点击对应行右侧的「AI降重」按钮，或点击顶部「全部降重」批量执行。
3.  系统将根据您的「系统配置」调用AI模型进行文本改写。
4.  降重完成后，结果直接内联展示，可点击「复制」或「查看全文」。

## 注意事项
1.  若不使用Cloudflare Worker的后备默认配置进行AI降重，首次使用前，请务必在「系统配置」中设置正确的API信息。
2.  目前查重报告仅支持HTML格式文件上传。「论文降重」支持大雅、PaperYY 格式；「AIGC检测」支持 PaperYY 格式；「AI智能解析」兼容任意格式。
